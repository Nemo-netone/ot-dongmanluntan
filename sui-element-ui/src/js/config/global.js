...
Vue.prototype.formatUrl = function (url) {
  if (url != null && url != undefined && url != "") {
    if (url.indexOf("http") > -1) {
      ...
      return withApiBase(fileBasePath + "/" + url)
    }
  }
}
...
//敏感词过滤
Vue.prototype.filterWords = function (content) {
  const sensitiveWords = ["暴力", "色情", "政治", "隐私", "机密"];
  const regex = new RegExp(sensitiveWords.map(word => word).join('|'), 'gi');
  let filterContent = content.replace(regex, match => '**'.repeat(match[0].length));
