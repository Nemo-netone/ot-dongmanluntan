...
<script>
  ...
  export default {
    ...
    computed: {
      uploadAction() {
        return withApiBase('/admin/sysAttach/uploadFile/image/' + this.type)
      }
    },
    watch: {
      value(value){
    ...
    methods: {
      ...
      beforeAvatarUpload(file) {
        ...
        const isImg = (file.type === 'image/png' || file.type === 'image/jpeg'|| file.type === 'image/gif'|| file.type === 'image/webp');
        const isLt5M = file.size / 1024 / 1024 < 5;
        if (!isImg) {
          this.$message.error('只能上传 jpg|png|webp|gif 格式的图片！');
          return false
        }
        if (!isLt5M) {
