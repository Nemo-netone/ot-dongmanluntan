...
<script>
  ...
  export default {
    ...
    computed: {
      uploadAction() {
        return withApiBase('/admin/sysAttach/uploadFile/image/editor')
      }
    },
    mounted () {
      this.addQuillTitle();
    ...
    methods: {
      ...
      handleAvatarSuccess(res, file) {
        if (res.isOk) {
          ...
          if(file.raw.type=='image/png'||file.raw.type=='image/jpeg'||file.raw.type=='image/gif'||file.raw.type=='image/webp'){
            quill.insertEmbed(length, 'image', this.formatUrl(res.obj.savePath))
          }else if(file.raw.type=='video/mp4'){
            quill.insertEmbed(length, 'video', this.formatUrl(res.obj.savePath))
          }
          // 调整光标到最后
          quill.setSelection(length + 1);
        } else {
          // 提示信息，需引入Message
          this.showWarn("文件上传失败");
        }
      },
      beforeAvatarUpload(file) {
        ...
        if (!isLt100M) {
          this.$message.error('上传头像文件大小不能超过 100MB！');
          return false
        }
        return true;
      }
