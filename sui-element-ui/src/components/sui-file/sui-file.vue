<template>
  <div id="file" class="body">
    <el-empty v-if="disabled&&showEmpty" description="暂无文件" class="empty-file"></el-empty>
    <el-upload
...
<script>
  import {withApiBase} from '@/js/config/api'
  ...
  export default {
    ...
    computed: {
      uploadAction() {
        return withApiBase('/admin/sysAttach/uploadFile/file/' + this.type)
      }
    },
    model:{
      prop:'file',
    ...
    methods: {
      ...
      handleDownload(file) {
        ...
        a.href = withApiBase("/admin/sysAttach/downFile?id="+file.id);
        // 触发 a 的点击事件
      ...
      handlePreview(file) {
        ...
        if (/.(gif|jpg|jpeg|png|webp)$/.test(name)) {
          this.$refs.view.imgViewer(file,'file')
      ...
      getFileList(refId) {
        ...
        this.$request.getFileHistory({refId: refId}).then(res => {
          if (res.isOk) {
            ...
            that.showEmpty = that.fileList.length==0;
...
<style scoped>
  ...
  .body /deep/ .el-upload-list--picture-card .el-upload-list__item{
