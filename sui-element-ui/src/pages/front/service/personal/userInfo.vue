<!-- 【用户信息】弹窗页面 -->
<template>
  <el-dialog width="450px" class="front-dialog" center :title="dialog.title" :close-on-click-modal="false" :append-to-body="true" v-dialog-drag :visible.sync="dialog.open">
    <el-form :model="register" ref="userInfoForm" :rules="rules" label-width="100px">
      <el-form-item label="用户头像" prop="photo">
        <sui-photo type="register" v-model="register.photo" :disabled="dialog.disabled" :url="register.photo"></sui-photo>
      </el-form-item>
    ...
    <div class="dialog-footer">
      <el-button class="sub-btn" @click="submitForm()">保 存</el-button>
    </div>
  </el-dialog>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      submitForm() {
        const that = this;
        that.$refs.userInfoForm.validate((valid) => {
          if (valid) {//表达校验
      ...
      showDialog(title, disabled) {
        this.resetForm(this.$refs.userInfoForm);//重置表单
