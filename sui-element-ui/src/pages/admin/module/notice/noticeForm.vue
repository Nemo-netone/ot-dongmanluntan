...
<template>
  <div class="body">
    <el-dialog :title="dialog.title" :close-on-click-modal="false" :visible.sync="dialog.open">
      <div v-if="pageFrom=='view'" class="edit-form">
        <h1 style="text-align: center">{{notice.title}}</h1>
        <el-image v-if="notice.picture" :src="formatUrl(notice.picture)"></el-image>
        <div style="margin-top: 10px">
          <el-tag size="small">
            <select-label :dict="noticeTypeDict" :value="notice.noticeType"></select-label>
          </el-tag>
          <el-tag size="small">{{notice.publishTime}}</el-tag>
        </div>
        <p v-html="notice.content"></p>
      </div>
      <el-form v-else :model="notice" ref="noticeForm" :rules="rules" label-width="120px" class="edit-form">
