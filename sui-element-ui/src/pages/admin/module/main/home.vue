<template>
  <el-container>
    <!-- 表单弹窗 -->
    <userInfo-form ref="userInfoDialog"/>
    <password-form ref="passwordDialog"/>
    <el-aside class="el-aside" :class="[{'aside-hidden': isHidden,'aside-horizontal':mode=='horizontal'}]">
      <el-menu
        ...
        <el-submenu v-if="menu.children&&!menu.href" v-for="(menu,index) in menuList" :key="index" :index="index">
          <template slot="title">
            <i style="color: white" :class="menu.icon"></i><span class="span" slot="title">{{menu.name}}</span>
          </template>
        ...
        <el-menu-item v-else class="el-submenu__title" @click="addTab(menu)" :index="menu.href">
          <i style="color: white" :class="menu.icon"></i><span slot="title" class="span">{{menu.name}}</span>
        </el-menu-item>
      </el-menu>
    </el-aside>
    <el-container style="height: 100vh;" :class="[{'body-horizontal':mode=='horizontal'}]">
      <el-header class="el-header" :class="[{'header-horizontal':mode=='horizontal'}]">
        ...
        <el-dropdown>
          <span v-if="userInfo" class="el-dropdown-link pointer">
            <el-image class="head-img" v-if="userInfo.photo" :src="formatUrl(userInfo.photo)"></el-image>
            <span style="color: white;font-weight: bold;font-size: medium">{{userInfo.userName}}</span>
            <i class="el-icon-arrow-down" style="color: white"></i>
          </span>
          <el-dropdown-menu slot="dropdown">
            <el-dropdown-item @click.native="openUserInfoDialog({title:'个人信息',pageFrom:'edit',id:userInfo.id})">个人信息</el-dropdown-item>
        ...
        <el-dropdown>
          <span class="el-dropdown-link pointer">
            <span v-if="userInfo.roleList" style="color: ghostwhite" class="pointer" v-for="role in userInfo.roleList" :key="role.id">【{{role.roleName}}】</span>
          </span>
          <el-dropdown-menu slot="dropdown">
            <el-dropdown-item v-if="userInfo.office">{{userInfo.office.name}}</el-dropdown-item>
          </el-dropdown-menu>
        </el-dropdown>
      </el-header>
      ...
      <el-footer class="copyright">Copyright {{projectName}} All Rights Reserved</el-footer>
    </el-container>
  </el-container>
</template>
<script>
...
<style scoped lang="scss">
  ...
  .el-header {
    ...
    text-align: right;
    font-size: 12px;
    background: lighten($theme-admin-color,5%)!important;
    padding: 0 10px;
    line-height: 60px;
  }
  .el-aside {
