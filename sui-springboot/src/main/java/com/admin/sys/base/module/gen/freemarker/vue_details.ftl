...
<div class="body">
  <div class="title">
    <div class="title-content">
      <uni-icons class="image left" custom-prefix="iconfont" type="icon-label" size="45"></uni-icons>
      <span class="title-span">${comments}详情</span>
      <uni-icons v-if="${className?uncap_first}.id&&!disabled" class="image right" custom-prefix="iconfont" type="icon-del" size="45" @click="del${className}(${className?uncap_first}.id)"></uni-icons>
    </div>
  </div>
  <div class="list-body">
    ...
    <#if attr.inputType == 'editor'>
      <uni-forms-item name="${attr.columnFieldName}" label="${attr.columnDesc}">
        <sui-editor :disabled="disabled" v-model="${className?uncap_first}.${attr.columnFieldName}" placeholder="请输入${attr.columnDesc}"></sui-editor>
      </uni-forms-item>
    </#if>
    ...
    <#if attr.inputType == 'date'>
      <uni-forms-item name="${attr.columnFieldName}" label="${attr.columnDesc}">
        <uni-datetime-picker :inputBorder="false" :disabled="disabled" v-model="${className?uncap_first}.${attr.columnFieldName}" type="date" :start="getDate().fullDate" placeholder="请选择${attr.columnDesc}"/>
      </uni-forms-item>
    </#if>
    </#if>
    </#list>
    ...
    <#if attr.inputType == 'associate'||attr.inputType == 'office'>
      ...
      submitForm(form) {
        ...
        that.$refs[form].submit().then(res => {
          that.showConfirm("确认提交吗?", function () {
