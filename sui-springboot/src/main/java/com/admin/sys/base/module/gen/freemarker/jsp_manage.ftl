    ...
    <#if attr.inputType == 'date'>
      ...
      <el-form-item label="${attr.columnDesc}" prop="${attr.columnFieldName}">
        <el-select v-model="queryForm.${attr.columnFieldName}" placeholder="请选择${attr.columnDesc}" size="small" filterable clearable>
          <el-option v-for="<#list "${attr.columnName}"?split("_") as codename><#if codename_index == 0>${codename}</#if></#list> in <#list "${attr.columnName}"?split("_") as codename><#if codename_index == 0>${codename}</#if></#list>Dict" :key="<#list "${attr.columnName}"?split("_") as codename><#if codename_index == 0>${codename}</#if></#list>.id" :value="<#list "${attr.columnName}"?split("_") as codename><#if codename_index == 0>${codename}</#if></#list>.id" :label="<#list "${attr.columnName}"?split("_") as codename><#if codename_index == 0>${codename}</#if></#list>.<#list "${attr.associateType}"?split(":") as codename><#if codename_index == 1>${codename}</#if></#list>"/>
        </el-select>
      </el-form-item>
      </#if>
    </#if>
...
<#list attrs as attr>
   <#if attr.isPk != 1 &&attr.isShow==1>
    ...
    <#if attr.inputType == 'select'>
      <el-table-column label="${attr.columnDesc}" prop="${attr.columnFieldName}" sortable>
        <template slot-scope="scope">
          <select-label :dict="${attr.columnFieldName}Dict" :value="scope.row.${attr.columnFieldName}"></select-label>
        </template>
      </el-table-column>
    </#if>
    ...
    <#if attr.inputType == 'associate'>
      <el-table-column label="${attr.columnDesc}" prop="${attr.columnFieldName}" sortable>
        <template slot-scope="scope">
          <associate-label :dict="<#list "${attr.columnName}"?split("_") as codename><#if codename_index == 0>${codename}</#if></#list>Dict" :value="scope.row.${attr.columnFieldName}" label="<#list "${attr.associateType}"?split(":") as codename><#if codename_index == 1>${codename}</#if></#list>"></associate-label>
        </template>
      </el-table-column>
    </#if>
  </#if>
    ...
    export default {
      ...
      data() {
        ...
        <#if attr.inputType == 'associate'||attr.inputType == 'office'>
          <#list "${attr.columnName}"?split("_") as codename><#if codename_index == 0>${codename}</#if></#list>Dict: [],
        </#if>
        </#list>
          dataList: [],
          queryForm: {
      ...
      mounted() {
        ...
        <#if attr.inputType == 'associate'>
        //初始化【${attr.columnDesc}】
        this.get<#list "${attr.columnName}"?split("_") as codename><#if codename_index == 0>${codename?cap_first}</#if></#list>Dict();
        </#if>
        </#list>
      },
      computed: {
      ...
      methods: {
        ...
        <#if attr.inputType == 'associate'>
        //【${attr.columnDesc}】
        get<#list "${attr.columnName}"?split("_") as codename><#if codename_index == 0>${codename?cap_first}</#if></#list>Dict() {
          this.$request.doGet('/admin/<#list "${attr.associateType}"?split(":") as codename><#if codename_index == 0>${codename}</#if></#list>/getList'<#list "${attr.associateType}"?split(":") as codename><#if codename == 'sysUser'>, {roleCode:'userRole'}</#if></#list>).then(res => {
            if (res.isOk) {
              this.<#list "${attr.columnName}"?split("_") as codename><#if codename_index == 0>${codename}</#if></#list>Dict = res.list;
            }
          });
        },
        </#if>
