  ...
  <div class="body">
    ...
    <#list attrs as attr>
      ...
      <#if attr.inputType == 'editor'>
        <el-form-item<#if attr.isHidden==1> class="hidden"</#if> label="${attr.columnDesc}" prop="${attr.columnFieldName}" class="sui-editor-item">
  ...
  export default {
    ...
    methods: {
      ...
      open(option) {
        ...
        <#list attrs as attr>
        <#if attr.inputType == 'associate'>
        this.get<#list "${attr.columnName}"?split("_") as codename><#if codename_index == 0>${codename?cap_first}</#if></#list>Dict();//加载【${attr.columnDesc}】
        </#if>
        </#list>
