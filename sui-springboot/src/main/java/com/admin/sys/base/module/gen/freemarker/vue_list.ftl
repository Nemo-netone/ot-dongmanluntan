  ...
  <div class="body">
    <div class="title">
      <div class="title-content">
        <uni-icons class="image left" custom-prefix="iconfont" type="icon-list" size="45"></uni-icons>
        <span class="title-span">${comments}列表</span>
        <uni-icons class="image right" custom-prefix="iconfont" type="icon-add" size="45" @click="toPage('/pages/module/${className?uncap_first}/${className?uncap_first}Details?pageFrom=add')"></uni-icons>
      </div>
    </div>
    <div class="list-body">
  ...
  export default {
    ...
    <#list attrs as attr>
      ...
      <#list "${attr.associateType}"?split(":") as codename><#if codename_index == 0>${codename}</#if></#list>Name(id) {
        if(this.<#list "${attr.associateType}"?split(":") as codename><#if codename_index == 0>${codename}</#if></#list>Dict.length>0&&id){
      ...
      getList() {
        ...
        let that = this;
        that.$request.doGet('/api/${className?uncap_first}/getPage',{page:that.page,size:that.size}).then(res=>{
          uni.hideLoading();
