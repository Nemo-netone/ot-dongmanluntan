...
- 完整全栈链路：前台门户、后台管理、Spring Boot API、Supabase PostgreSQL 数据库和线上部署都已接通。
- 双端演示清晰：普通用户可以浏览、评论和收藏内容；管理员可以在后台维护文章、分类、公告、用户和系统配置。
- 数据隔离明确：线上数据放在 Supabase 独立 schema `ot_dongmanluntan`，不占用 `public`，方便和其他项目共用同一个 Supabase 实例。
- 部署结构稳定：前端固定发布到 Cloudflare Pages `main` 分支，后端固定运行在 CloudBase Run，演示地址后续更新仍保持不变。
- 公开仓库脱敏：只提交演示数据、占位环境变量和非敏感部署配置，不保存真实 token、数据库密码或云密钥。

## 建议体验路径

| 场景 | 入口 | 可以验证的能力 |
|---|---|---|
| 前台浏览 | `/front/home` | 首页轮播、动漫文章、分类、公告和图片资源 |
| 用户互动 | 前台登录后进入文章详情 | 评论、回复、收藏、浏览记录和个人信息 |
| 后台管理 | `/login` | 内容管理、分类管理、公告管理、用户管理和系统配置 |
| API 健康检查 | CloudBase API `/api/health` | 后端服务是否在线 |
| 数据接口 | CloudBase API `/api/category/getList` | Supabase 数据库连接和 schema 数据读取 |
